 ...
 */
@Directive({
    selector: '[qdooraScrollbar]',
    exportAs: 'qdooraScrollbar',
    standalone: true,
})
export class QdooraScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_qdooraScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() qdooraScrollbar: boolean = true;
    @Input() qdooraScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('qdooraScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.qdooraScrollbar = coerceBooleanProperty(
                changes.qdooraScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.qdooraScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('qdooraScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.qdooraScrollbarOptions.currentValue
            );

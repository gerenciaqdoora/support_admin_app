...
import { QdooraDrawerService } from '@core/components/drawer/drawer.service';
import {
    QdooraDrawerMode,
    QdooraDrawerPosition,
} from '@core/components/drawer/drawer.types';
import { QdooraUtilsService } from '@core/services/utils/utils.service';

@Component({
    selector: 'qdoora-drawer',
    templateUrl: './drawer.component.html',
    styleUrls: ['./drawer.component.scss'],
    encapsulation: ViewEncapsulation.None,
    exportAs: 'qdooraDrawer',
    standalone: true,
})
export class QdooraDrawerComponent implements OnChanges, OnInit, OnDestroy {
    ...
    @Input() position: QdooraDrawerPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly fixedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output() readonly modeChanged: EventEmitter<QdooraDrawerMode> =
        new EventEmitter<QdooraDrawerMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<QdooraDrawerPosition> =
        new EventEmitter<QdooraDrawerPosition>();
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'qdoora-drawer-animations-enabled': this._animationsEnabled,
            'qdoora-drawer-fixed': this.fixed,
            'qdoora-drawer-hover': this._hovered,
            [`qdoora-drawer-mode-${this.mode}`]: true,
            'qdoora-drawer-opened': this.opened,
            [`qdoora-drawer-position-${this.position}`]: true,
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    ngOnInit(): void {
        // Register the drawer
        this._qdooraDrawerService.registerComponent(this.name, this);
    }
    ...
    ngOnDestroy(): void {
        ...
        this._qdooraDrawerService.deregisterComponent(this.name);
    }
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('qdoora-drawer-overlay');

        // Add a class depending on the fixed option
        if (this.fixed) {
            this._overlay.classList.add('qdoora-drawer-overlay-fixed');
        }

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add('qdoora-drawer-overlay-transparent');
        }

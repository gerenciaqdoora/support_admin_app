...
import {
    QdooraNavigationItem,
    QdooraVerticalNavigationAppearance,
    QdooraVerticalNavigationMode,
    QdooraVerticalNavigationPosition,
} from '@core/components/navigation/navigation.types';
import { QdooraVerticalNavigationAsideItemComponent } from '@core/components/navigation/vertical/components/aside/aside.component';
...
import { QdooraVerticalNavigationGroupItemComponent } from '@core/components/navigation/vertical/components/group/group.component';
...
import { QdooraScrollbarDirective } from '@core/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'qdoora-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: qdooraAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'qdooraVerticalNavigation',
    standalone: true,
    imports: [
        QdooraScrollbarDirective,
        QdooraVerticalNavigationAsideItemComponent,
        ...
        QdooraVerticalNavigationGroupItemComponent,
        QdooraVerticalNavigationSpacerItemComponent,
    ],
})
export class QdooraVerticalNavigationComponent
    ...
    @Input() appearance: QdooraVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: QdooraVerticalNavigationMode = 'side';
    @Input() name: string = this._qdooraUtilsService.randomId();
    @Input() navigation: QdooraNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: QdooraVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<QdooraVerticalNavigationAppearance> =
        new EventEmitter<QdooraVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<QdooraVerticalNavigationMode> =
        new EventEmitter<QdooraVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<QdooraVerticalNavigationPosition> =
        new EventEmitter<QdooraVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<QdooraNavigationItem> =
        new ReplaySubject<QdooraNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<QdooraNavigationItem> =
        new ReplaySubject<QdooraNavigationItem>(1);
    ...
    private _qdooraScrollbarDirectives!: QueryList<QdooraScrollbarDirective>;
    private _qdooraScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'qdoora-vertical-navigation-animations-enabled':
                this._animationsEnabled,
            [`qdoora-vertical-navigation-appearance-${this.appearance}`]: true,
            'qdoora-vertical-navigation-hover': this._hovered,
            'qdoora-vertical-navigation-inner': this.inner,
            'qdoora-vertical-navigation-mode-over': this.mode === 'over',
            'qdoora-vertical-navigation-mode-side': this.mode === 'side',
            'qdoora-vertical-navigation-opened': this.opened,
            'qdoora-vertical-navigation-position-left': this.position === 'left',
            'qdoora-vertical-navigation-position-right':
                this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for qdooraScrollbarDirectives
     */
    @ViewChildren(QdooraScrollbarDirective)
    set qdooraScrollbarDirectives(
        qdooraScrollbarDirectives: QueryList<QdooraScrollbarDirective>
    ) {
        // Store the directives
        this._qdooraScrollbarDirectives = qdooraScrollbarDirectives;

        // Return if there are no directives
        if (qdooraScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._qdooraScrollbarDirectivesSubscription) {
            this._qdooraScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._qdooraScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                qdooraScrollbarDirectives.forEach((qdooraScrollbarDirective) => {
                    qdooraScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                const activeItem =
                    this._navigationContentEl.nativeElement.querySelector(
                        '.qdoora-vertical-navigation-item-active'
                    );
            ...
            // Otherwise
                ...
                this._qdooraScrollbarDirectives.forEach(
                    (qdooraScrollbarDirective) => {
                        // Skip if not enabled
                        if (!qdooraScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        qdooraScrollbarDirective.scrollToElement(
                            '.qdoora-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: QdooraNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: QdooraNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('qdoora-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'qdoora-vertical-navigation-overlay-transparent'
            );
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add(
            'qdoora-vertical-navigation-aside-overlay'
        );
